...
const helpers_admin = require('../config/helpers_admin');


// LOGIN ROUTE
router.post('/login', [helpers_admin.hasAuthFields, helpers_admin.isPasswordAndUserMatch], (req, res) => {
    let token = jwt.sign({state: 'true', email: req.body.email, username: req.body.username}, helpers_admin.secret, {
        algorithm: 'HS512',
        expiresIn: '4h'
    });
...
router.post('/register', [
    ...
    body('email').custom(value => {
        return helpers_admin.database.table('admin').filter({
            $or:
                [
    ...
    if (!errors.isEmpty()) {
         ...
         * ROLE 555 = CUSTOMER
         **/
         helpers_admin.database.table('admin').insert({
            username: username,
            password: password,
            email: email,
            role: 555
